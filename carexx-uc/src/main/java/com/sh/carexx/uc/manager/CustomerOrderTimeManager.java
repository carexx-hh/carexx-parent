...
public class CustomerOrderTimeManager {
    ...
    public void add(CustomerOrderTimeFormBean customerOrderTimeFormBean) throws BizException {
        ...
        Date startTime = null;
        if (ValidUtils.isDateTime(customerOrderTimeFormBean.getStartTime())) {
            startTime = DateUtils.toDate(customerOrderTimeFormBean.getStartTime(),
                    DateUtils.HH_MM_SS);
        }
        Date endTime = null;
        if (ValidUtils.isDateTime(customerOrderTimeFormBean.getEndTime())) {
            endTime = DateUtils.toDate(customerOrderTimeFormBean.getEndTime(),
                    DateUtils.HH_MM_SS);
        }
